...
<div class="container-fluid dark-nav">
    <div class="row">
        <div class="col-lg-12 md-4 posicionamiento_flotante">
            ...
            <div class="card shadow mb-4">
                <div class="card-header text-white" style="background-color: #621132;"><i class="fas fa-map-marked-alt"></i> <b>Seguimiento de la Bitácora de recorrido</b></div>
                <div class="card-body">
                    <div class="form-row">
                        <div class="bg-light form-inline" style="padding:15px 0 15px 0; text-indent:1.8em; line-height: 2.1em;">
                            ...
                            <span>Puesto Resguardante:&nbsp;<strong>{{ $solDetalle->puesto_resguardante_unidad }}</strong></span>
                    ...
                    {{-- abrir formulario --}}
                    {!! Form::open(['method' => 'POST', 'enctype' => 'multipart/form-data', 'id' => 'frmBitacoraRecorrido', 'autocomplete' => 'off']) !!}
                    <div class="form-row">
                        <div class="col-md-2">
                            {!! Form::label('chkcomisionLabel', '¿Es comisionado?', ['id' => 'chkcomisionLabel'])!!}
                            <div class="custom-file">
                                <div class="form-switch">
                                    <input class="form-check-input" name="chkcomision" type="checkbox" role="switch" id="chkcomision" {{ ($esComision == 1) ? 'checked' : '' }}>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            ...
                            <div class="custom-file">
                                {!! Form::text('numero_comision',null, ['class' => 'form-control', 'id' => 'numero_comision', ($esComision == 1) ? '' : 'readonly' ]) !!}
                            </div>
                        </div>
                        <div class="col-md-3">
                            {!! Form::label('facturalabel', 'N° Factura', ['id' => 'facturalabel'])!!}
                            <div class="custom-file">
                                {!! Form::text('numero_factura',null, ['class' => 'form-control', 'id' => 'numero_factura', ($esComision == 1) ? '' : 'readonly' ]) !!}
                            </div>
                        </div>
                        <div class="col-md-3">
                            {!! form::label('rendimientovehiculolabel', 'Rendimiento Vehículo (km * lts)', ['id'=> 'rendimientovehiculolabel']) !!}
                            <div class="custom-file">
                            ...
                            <select id="folio_inicial_final" name="folio_inicial_final[]" multiple>
                                @if (count($getFolios) > 0)
                                    @foreach ($getFolios as $key => $value)
                                        <option value="{{ $value->id."|".$value->denominacion }}" {{ ($value->utilizado == 1) ? 'selected' : ''  }}>{{ $value->numero_folio}}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <div class="form-row">
                        <div class="col-md-2 mb-3">
                            <div class="form-group">
                                ...
                                {!! Form::text('litros','', ['class' => 'form-control', 'id' => 'litros', 'onkeypress' => 'return isNumberKey(event, this)']) !!}
                            </div>
                        </div>
                        <div class="col-md-2 mb-3">
                            <div class="form-group">
                                {!! Form::label('precioporlitroLabel', 'Precio x Litro (gasolina)')!!}
                                {!! Form::text('precioporlitro','', ['class' => 'form-control', 'id' => 'precioporlitro', 'onkeypress' => 'return isNumberKey(event, this)']) !!}
                            </div>
                        </div>
                        <div class="col-md-2 mb-3">
                            <div class="form-group">
                                {!! Form::label('importeLabel', 'Importe gasolina')!!}
                                {!! Form::text('importe','', ['class' => 'form-control', 'id' => 'importe', 'onkeypress' => 'return isNumberKey(event, this)']) !!}
                            </div>
                        </div>
                        <div class="col-md-2 mb-3">
                            <div class="form-group">
                                {!! Form::label('importefoliosLabel', 'Importe folios')!!}
                                {!! Form::text('importefolios','', ['class' => 'form-control', 'id' => 'importefolios',  'onkeypress' => 'return isNumberKey(event, this)', ($esComision == 1) ? '' : 'readonly']) !!}
                            </div>
                        </div>
                        <div class="col-md-4 mb-3">
                            <div class="form-group">
                    ...
                    <div class="custom-file">
                        {!! Form::button('<i class="fas fa-plus-square"></i> Agregar Recorrido',['type'=>'submit', 'rel' =>'noopener noreferrer', 'class' => 'btn btn-success', 'id' => 'frmsubmit']) !!}
                    </div>
                    {!! Form::hidden('idCatVehiculo', base64_encode($idSolPre), ['class'=> 'form-control', 'id' => 'idCatVehiculo']) !!}
    ...
    <div class="row">
        <div class="col-lg-12 md-4 posicionamiento_flotante">
            <div class="card shadow mb-4">
                <div class="card-header text-white" style="background-color: #7393B3"><i class="fas fa-route"></i> <b>Recorrido</b></div>
                <div class="card-body">
                   {{-- tabla --}}
                   <table style="font-size: 12px; padding: 2px 2px;">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                           ...
                           @if (count($getTemporalBitacora) > 0)
                            @foreach ($getTemporalBitacora as $k => $v)
                                ...
                                @endphp
                                <tr style="background-color: {{ $v->comision == true ? '#AAB7B8' : '' }}; color: {{ $v->comision == true ? '#FFF' : '' }}">
                                    <td scope="col" style="width: 80px;">{{ $fecha }}</td>
                                    <td scope="col" style="width: 60px;">{{ $v->kilometraje_inicial }}</td>
                                    <td scope="col" style="width: 150px;">{{ $v->actividad_inicial }}</td>
                                    <td scope="col" style="width: 150px;">{{ $v->actividad_final }}</td>
                                    <td scope="col" style="width: 5px;">{{ $v->kilometraje_final }}</td>
...
@section('contenidoJavaScript')
    ...
    <script>
        ...
        if (check === true) {
            // si está checado
            selectOptionValue = $('#folio_inicial_final option:selected').toArray().map(item => item.value); // asignamos la variable
        }
    </script>
           ...
           $('#precioporlitro').on("blur", function(){
            ...
            $('#frmBitacoraRecorrido').validate({
                ...
                rules: {
                    ...
                    de: {required: true},
                    a: {required: true},
                    folio_inicial_final: {required: true},
                    litros: {required: true},
                    precioporlitro: {required: true}

                },
                messages: {
                    fechaEntrada: {required: "Fecha requerida"},
                    kmInicial:{required: "Kilometro Inicial requerido"},
                    kmFinal: {required: "Kilometro Final requerido"},
                    de: {required: "Origen es requerido"},
                    a: {required: "Destino es requerido"},
                    litros: {required: "Litros es requerido"},
                    precioporlitro: {required: "Precio por Litro requerido"}
                },
                ...
                submitHandler: async function(frm, evt){
                    ...
                    await $.ajax({
                        ...
                        beforeSend: function () {
                            ...
                            $('#frmBitacoraRecorrido').attr('disabled', 'disabled'); // deshabilitar el formulario
                        ...
                        success: function(response){
                            ...
                            const timmer = setInterval(() =>{
                                per = per + 50;
                                spinnerProgress(per, timmer, formularioAsignado, response.res, response.message);
                            }, 500);
                        },
