        ...
        <div class="col-lg-10 mb-4">
            <div class="bd-example">
                ...
                <div class="row g-2">
                    <div class="col-md">
                        <div class="form-floating">
                            {!! Form::text('area_solicita', '', [
                                'class' => 'form-control',
                                'required',
                                'autocomplete' => 'off',
                                'id' => 'area_solicitud',
                            ]) !!}
                            {!! Form::label('area_solicita', 'Área que Solicita', ['for' => 'area_solicita', 'class' => 'control-label', 'typeahead']) !!}
                    ...
                    <div class="col-md">
                        <div class="form-floating mb-3">
                            {!! Form::select('depto', ['L' => 'LISTAS', 'S' => '-- SELECCIONAR --'], 'S', [
                                'class' => 'form-control',
                                'id' => 'deptos',
                            ]) !!}
                    ...
                    <div class="col-md">
                        <div class="form-floating">
                            ...
                            {!! Form::label('fechaRequisicion', 'Fecha de Requisición', [
                                'for' => 'fechaRequisicion',
                                'class' => 'control-label',
                            ]) !!}
                            <div class="invalid-feedback">
                                Proveer Fecha
                ...
                <div class="row g-2">
                    ...
                    <div class="col-md">
                        <div class="form-floating mb-3">
                            {!! Form::text('puesto_solicita', '', [
                                'class' => 'form-control',
                                'required',
                                'id' => 'solicita',
                                'autocomplete' => 'off',
                            ]) !!}
                            {!! Form::label('puesto_solicita_label', 'Puesto Solicita', [
                                'for' => 'puesto_solicita',
                                'class' => 'control-label',
                            ]) !!}
                            <div class="invalid-feedback">
                    ...
                    <div class="col-md">
                        <div class="form-floating">
                            ...
                            {!! Form::label('puesto_autoriza_label', 'Puesto Autoriza', [
                                'for' => 'puesto_autoriza',
                                'class' => 'control-label',
                            ]) !!}
                            <div class="invalid-feedback">
                ...
                <hr>
                {!! Form::button('<i class="fas fa-edit" aria-hidden="true"></i> Enviar', [
...
@section('contenidoJavaScript')
    ...
    <script>
        ...
        (() => {
            ...
            btnAdd.addEventListener('click', event => {
                if (contador < max_fields) {
                    ...
                    $('#addNewComponent').append(
                        ...
                        '<input type="text" name="itemPartida[' + contador +
                        '][partida_presupuestal]" id="partida_presupuestal_' + contador +
                        '" autocomplete="off"  class="form-control autocom"/>' +
                        '</td>' +
                        '<td data-label="CONCEPTO">' +
                        '<input type="text" name="itemPartida[' + contador + '][concepto]" id="concepto_' +
                        contador + '" autocomplete="off"  class="form-control"/>' +
                        '</td>' +
                        '<td data-label="AGREGAR">' +
                        '<a href="javascript:;" class="btn btn-success btn-circle btn-sm addItemPartidaPresupuestal" id="' +
                        contador + '">' +
                        ...
                        '</td>' +
                        '</tr>' +
                        '<input type="hidden" name="itemPartida[' + contador +
                        '][id_partida_presupuestal]" id="id_partida_' + contador + '"/>' +
                        '<tr>' +
                        '<td colspan="4">' +
            ...
            $(document).on('click', '.addItemPartidaPresupuestal', function(e) {
                ...
                if (i < max_fields) {
                    ...
                    $('#addItemPartida_' + id).append(
                        ...
                        '<tr>' +
                        '<td data-label="CANT.">' +
                        '<input type="text" name="itemPartida[' + id + '][' + i +
                        '][cantidad]" class="form-control cantidades" id="cantidad[]" autocomplete="off"/>' +
                        '</td>' +
                        '<td data-label="UNIDAD">' +
                        '<input type="text" name="itemPartida[' + id + '][' + i +
                        '][unidad]" class="form-control" id="unidad[]" autocomplete="off"/>' +
                        '</td>' +
                        '<td data-label="DESCRIPCIÓN">' +
                        '<textarea name="itemPartida[' + id + '][' + i +
                        '][descripcion]" id="descripcion[]" class="form-control" rows="2" cols="30"></textarea>' +
                        '</td>' +
                        '<td data-label="QUITAR">' +
                        '<button id="EliminarElemento_' + id + '_' + i +
                        '" class="btn-close remove-item-tr" arial-label="Close"></button>' +
                        '</td>' +
                        '</tr>' +
                        '</tbody>' +
                        '</table>'
                    ...
                    if (itemId) {
                        ...
                        $.get(url, function(data) {
                            $.each(data, function(fetch, Obj) {
                                $('#deptos').append('<option value="' + Obj.id + '">' + Obj
                                    .nombre + '</option>');
                            });
                        })
                    } else {
            ...
            var path = "{{ route('area.solicita.requisicion') }}";
