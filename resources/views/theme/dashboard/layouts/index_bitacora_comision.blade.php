...
@section('title', 'Indice de Bitácoras de Comisión | SISCOM by ICATECH')
        ...
        @if (auth()->user()->can('publicar bitacora'))
            <div class="col-xl-3 col-md-6 mb-4">
                <div class="card border-left-primary shadow h-100 py-2">
                    <a href="{{ route('solicitud.bitacora.comision.create') }}">
                        <div class="card-body">
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">
                            ...
                            SOLICITUDES DE BITÁCORAS DE COMISIONES
                        </h6>
                    </div>
                    <div class="card-body">
                        {{-- Agregamos Información de una tabla --}}
                        <table>
                            <caption>Bitácora de comisión guardada</caption>
                            <thead>
                              <tr>
                            ...
                            <tbody>
                                @if (count($temporal_comision) > 0)
                                    @foreach ($temporal_comision as $item_comision_temporal => $v)
                                        <tr>
                                            <td data-label="periodo">{{ $v->periodo }}</td>
                                            <td data-label="Placas">{{ $v->placas }}</td>
                                            <td data-label="Seguir Escribiendo">
                                                <a href="{{ route('comision.pre.guardado', base64_encode($v->id)) }}" class="btn btn-warning btn-circle btn-sm">
