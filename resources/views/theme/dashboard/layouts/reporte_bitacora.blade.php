    ...
    <div class="row">
        <div class="col-lg-12 md-4 posicionamiento_flotante">
            ...
            <div class="card shadow mb-4">
                <div class="card-header text-white" style="background-color: #621132;"><i class="fas fa-search"></i> <b>Busqueda por Factura</b></div>
                <div class="card-body">
                    {{-- abrir formulario --}}
                    {!! Form::open(['method' => 'POST', 'enctype' => 'multipart/form-data', 'id' => 'frmBitacoraReporte']) !!}
                        ...
                        <br>
                        {!! Form::button('<i class="fas fa-filter"></i> Filtrar', ['rel' =>'noopener noreferrer', 'class' => 'btn btn-info', 'id' => 'frmsubmit']); !!}
                    {!! Form::close() !!}
                    {{-- cerrar formulario --}}
                </div>
            </div>
...
{{-- bitacora de recorrido --}}
    <div class="row">
        <div class="col-lg-12 md-4 posicionamiento_flotante">
            <div class="card shadow mb-4">
                <div class="card-header text-white" style="background-color: #621132"><i class="fas fa-layer-group"></i> <b>Bitácora</b></div>
                <div class="card-body">
                   {{-- tabla --}}
                   <table id="log">
            ...
            $('#facturas').tokenfield({
                autocomplete: {
                    source: function(request, response){
                        ...
                        $.get(routesUrl, {query: request.term},  function(data){
                            data = $.parseJSON(data);
                            response(data);
                        });
                    },
                ...
                if (factura.length > 0) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(){
                            ...
                            $('#frmBitacoraReporte').attr('disabled', 'disabled'); // deshabilitar el formulario
                            $('#cover-spin').show(0);
                        },
                        success: function(response){
                            ...
                            const timmer = setInterval(() =>{
                                ...
                                spinnerProgress(percentage, timmer, formularioReporte, response.success, response.message);
                            }, 500);
                        },
                    ...
                    if (data == true) {
                        ...
                        setTimeout(() => {
                            ...
                            Object.entries(objeto).forEach(([key, value]) => {
                                let urlReq = "{{ route('generar.reporte.bitacora.pdf', ':id') }}";
                                ...
                                */
                                contenidoTabla +=   `<tr>`+
                                                        ...
                                                        `<td data-label="Módelo"><b>${value['modelo']}</b></td>`+
